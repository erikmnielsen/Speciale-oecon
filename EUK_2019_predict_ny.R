...
func_empprod <- function(method, type) {
  ...
  data = data %>% group_by_(country, code) %>% mutate(baseyear = EMP[year == 2000])
  ...
  if (type=="industrier") {
    ...
    pdata$emp_logchanges = diff(pdata$EMP, lag = 1, shift = "time")*100
    ...
    pdata$baseyear = ifelse()
    pdata$indicatorfunction = ifelse(pdata$indnr )
