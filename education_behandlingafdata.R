...
data = data %>% select(c(-Code_desc, -Gender, -Age, -Database, -Variable_desc, -IndNr, -Sort_ID)) %>% filter(Country %in% c("AT", "BE", "CZ", "DE", "DK", "EE", "EL", "FI", "FR", "HR", "HU", "IT", "JP", "LV", "NL", "PL", "PT", "RO", "SE", "SI", "SK")) %>% filter(Variable == "H_shares") %>% select(-Variable) %>% filter(Code %in% c("B","C","D","E","F","G","H","I","J","K","L","M_N")) 
