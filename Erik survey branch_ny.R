...
summary(lm(data_A1$B3 ~ data_A1$udgrp1 + data_A1$udgrp2 ))
...
regc1 = {svyglm(C3 ~ bra10grp_code2 +  bra10grp_code3 + bra10grp_code4 + udgrp + factor(Funktioner) + aldergrp + loengrp + F1 + F2 + G1 + G2 + G3 + virk,
               family=gaussian(),
  ...
  summary(regc1)
fit.svyglm(regc1, digits = 3)  

  step1 = {svyglm(C1 
                
                ~ bra10grp_code2 + bra10grp_code3 + bra10grp_code4 + udgrp + factor(Funktioner) + aldergrp + loengrp + F1 + F2 + G1 + G2 + G3,
                family=gaussian(), 
                design=svydesign_A1A5, 
                data=data_A1A5)}
summary(step1)
...
regc2 = {svyglm(F2 ~ factor(bra10grp_code1) + factor(udgrp) + factor(Functions) + factor(aldergrp) + factor(loengrp), 
               family=gaussian(), 
               design=svydesign_A1A5, 
               data=data_A1A5)}
...
regc3 = {svyglm(C3 ~ factor(bra10grp_code1) + factor(Functions) + factor(aldergrp) + factor(loengrp) + Leveremodtageoutput + Startovervågestopperobottter + Advancerettek1 + Advancerettek2 + Advancerettek3,
               family=gaussian(), 
               design=svydesign_A1A5, 
               data=data_A1A5)}
...
reg_d1 = svyglm(B3 ~ factor(bra10grp_code1) + factor(Funktioner) + factor(loengrp) + factor(aldergrp) + Leveremodtageoutput + Startovervågestopperobottter + Advancerettek1 + Advancerettek2 + Advancerettek3,
                family=gaussian(), 
                design=svydesign_A1, data=data_A1)
summary(reg_d1)

#Jeg mister muligvis mit arbejde inden for de næste 6 måneder
reg_d2 = svyglm(D2 ~ -1 + factor(bra10grp_code1) + factor(Functions) + factor(loengrp) + factor(aldergrp) + Leveremodtageoutput + Startovervågestopperobottter + Advancerettek1 + Advancerettek2 + Advancerettek3 + B7,
                family=gaussian(), 
                design=svydesign_A1, 
                data=data_A1)#I betragtning af alle mine bestræbelser og resultater i mit job, føler jeg, at jeg bliver betalt behørigt

reg_d3 = svyglm(D3 ~ -1 + factor(bra10grp_code1) + factor(Functions) + factor(loengrp) + factor(aldergrp) + Leveremodtageoutput + Startovervågestopperobottter + Advancerettek1 + Advancerettek2 + Advancerettek3,
                family=gaussian(), 
                design=svydesign_A1, 
                data=data_A1)
